...
#   None
#######################################
system_unzip_ivertionchat() {
  print_banner
  printf "${WHITE} 💻 Baixando ivertionchat...${GRAY_LIGHT}"
  printf "\n\n"
...
criar_banco_dados() {
  ...
  sudo su - root <<EOF
docker run --name postgresql-${nome_instancia} -e POSTGRES_USER=ivertionchat -e POSTGRES_PASSWORD=${pg_pass} -e TZ="America/Sao_Paulo" -p ${porta_postgre_intancia}:5432 --restart=always -v /data:/var/lib/postgresql/data -d postgres
...
cat > /etc/nginx/conf.d/ivertionchatio.conf << 'END'
...
system_docker_start() {
  ...
  docker container start redis-ivertionchat
...
system_success() {
  ...
  printf "\n\n"
  printf "\n"
  printf "Usuário: admin@ivertionchat.io"
  printf "\n"
  printf "Senha: 123456"
  printf "\n"
  ...
  printf "\n"
  printf "Usuario do Banco de Dados: ivertionchat"
  printf "\n"
